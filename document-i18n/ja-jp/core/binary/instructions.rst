...
:ref:`インストラクション <syntax-instr>` は「opcode（operation code）」でエンコードされます。
個別のopcodeは1バイトで表され、インストラクションに即値引数が存在する場合はその後ろに即値引数が続きます。
唯一の例外は :ref:`構造化制御インストラクション <binary-instr-control>` で、これはネストしたインストラクションシーケンスを囲む複数のopcodeで構成されます。

.. note::
   インストラクションをエンコードするバイトコード表には空欄がありますが、これらは将来の拡張のために予約されています。
...
:ref:`制御インストラクション <syntax-instr-control>` にはさまざまなエンコーディングがあります。構造化されたインストラクションの場合、ネストしたブロックを形成するインストラクションシーケンスは、|END| や |ELSE| を表す明示的なopcodeで終了します。

:ref:`ブロック型 <syntax-blocktype>` は2つの特殊な圧縮形式でエンコードされます。単一の :ref:`値型 <binary-valtype>` として空の型を表すバイト :math:`\hex{40}` か、正の :ref:`符号付き整数 <binary-sint>` としてエンコードされた1個の :ref:`型インデックス <binary-typeidx>` のいずれかになります。
...
.. note::
   |IF| インストラクションのエンコーディングに含まれる |ELSE| のopcode :math:`\hex{05}` は、それに続くインストラクションシーケンスが空の場合は省略可能です。

   :ref:`ブロック型 <syntax-blocktype>` の中にある :ref:`型 <syntax-typeidx>` インデックスは :ref:`他の型インデックス <binary-typeidx>` と異なり、正の :ref:`符号付き整数 <syntax-sint>` としてエンコードされます。これは、その |SignedLEB128| ビットパターンが :ref:`値型 <binary-valtype>` やその他の特殊コード :math:`\hex{40}` （これは負の整数の LEB128 エンコーディングに対応します）のエンコーディングと衝突しないためです。
   許されているインデックスの範囲でデータ喪失を回避するために、33ビットの符号付きで扱われています。

   WebAssemblyの今後のバージョンでは、|CALLINDIRECT| インストラクションのエンコーディングで発生するゼロバイトが、追加のテーブルをインデックス化するのに用いられる可能性があります。
...
:ref:`パラメーターインストラクション <syntax-instr-parametric>` は1個のバイトコードで表現されます。
...
:ref:`変数インストラクション <syntax-instr-variable>` は、バイトコードにそれぞれの :ref:`インデックス <syntax-index>` のエンコードを付けた形で表現されます。
...
:ref:`メモリーインストラクション <syntax-instr-memory>` のバリエーションごとに異なるバイトコードを用いてエンコードされます。ロードやストアの後ろにはその |memarg| 即値のエンコーディングが付けられます。
...
.. note::
   WebAssemblyの今後のバージョンでは、|MEMORYSIZE| インストラクションや |MEMORYGROW| インストラクションのエンコーディングで発生するゼロバイトが、追加のメモリーをインデックス化するのに用いられる可能性があります。
...
:ref:`数値インストラクション <syntax-instr-numeric>` のすべてのバリエーションは、異なるバイトコードで表現されます。

|CONST| インストラクションの後ろには、対応するリテラルが付けられます。
...
その他のすべての数値インストラクションは、即値を持たない純粋なopcodeです。
...
あふれを切り落とすインストラクションには必ず1バイトのプレフィックスが付きますが、実際のopcodeは可変長の :ref:`符号なし整数 <binary-uint>` でエンコードされます。
...
:ref:`式 <syntax-expr>` は、|END| を表す明示的な :math:`\hex{0B}` opcodeで終了するインストラクションシーケンスでエンコードされます。
